...
function getSimilarDrafts(verseRef: string, similarPairsCount: number, sourceBookWhitelist: string): Promise<any> {
    return Promise.resolve(
        ...
        vscode.commands.executeCommand('codex-editor-extension.pythonMessenger', 'getSimilarDrafts', verseRef, similarPairsCount, sourceBookWhitelist)
    );
}
...
async function getSurroundingContext(sourceTextFilePath: string, verseRef: string, contextSize: string): Promise<string> {
    ...
    try {
        ...
        const verseRefs = await getVerseRefs(sourceTextFilePath, book, chapter, verse, n);
...
async function getVerseRefs(sourceTextFilePath: string, book: string, chapter: number, verse: number, n: number): Promise<string[]> {
    ...
    for (let i = 0; i < Math.floor(n / 2); i++) {
        currentVerse++;
        const lastVerse = await getLastVerse(sourceTextFilePath, currentBook, currentChapter);
...
async function findTargetVerse(verseRef: string): Promise<string | null> {
    ...
    const targetDir = vscode.Uri.joinPath(workspaceFolders[0].uri, 'files', 'target');
    ...
    try {
        ...
        if (debugMode) {
            ...
            try {
                ...
                vscode.window.showInformationMessage(`Debug messages stored in ${messagesFilePath.fsPath}`, 'Disable Debug Mode')
                    .then(selection => {
