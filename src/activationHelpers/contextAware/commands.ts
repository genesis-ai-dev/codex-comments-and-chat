...
export async function registerCommands(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "translators-copilot.setMainChatLanguage",
      async () => {
        const options: vscode.QuickPickItem[] = Object.values(
          MainChatLanguage
        ).map((language) => ({
          label: language,
        }));
        ...
        if (selectedLanguage) {
          await vscode.workspace
            ...
            .update(
              "main_chat_language",
              selectedLanguage.label as MainChatLanguage,
              vscode.ConfigurationTarget.Workspace
            );
          vscode.window.showInformationMessage(
            `Main chat language set to ${selectedLanguage.label}`
          );
        }
      }
    )
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "translators-copilot.setLLMApiKey",
      async () => {
